...
async function migrateUserGroups(): Promise<void> {
  try {
    ...
    console.log(
      `Found ${userGroupsWithoutIsActive.length} UserGroup documents without is_active field`
    );

    if (userGroupsWithoutIsActive.length === 0) {
      console.log(
        "No documents need migration. All UserGroup documents already have is_active field."
      );
      return;
    }
    ...
    console.log(
      `Successfully updated ${updateResult.modifiedCount} UserGroup documents`
    );
    ...
    if (remainingWithoutIsActive.length === 0) {
      console.log(
        "✅ Verification passed: All UserGroup documents now have is_active field"
      );
    } else {
      console.log(
        `⚠️  Warning: ${remainingWithoutIsActive.length} documents still missing is_active field`
      );
    }
  } catch (error) {

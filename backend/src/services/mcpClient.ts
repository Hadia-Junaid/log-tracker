...
export class MCPClient extends EventEmitter {
  ...
  private async getUserContext(userId: string): Promise<UserContext | undefined> {
    try {
      ...
      return {
        ...
        atRiskApplications: await this.getAtRiskApplications(user._id.toString())
      };
    } catch (error) {
  ...
  /**
   * Get at-risk applications for a user
   */
  private async getAtRiskApplications(userId: string): Promise<Array<{
    appId: string;
    name: string;
    messages: string[];
  }>> {
    try {
      // Get at-risk rules
      const rules = await AtRiskRule.find().lean();
      if (!rules.length) {
        return [];
      }
      ...
      const assignedAppIds = [
        ...new Set(userGroups.flatMap(group => 
          group.assigned_applications.map(id => id.toString())
        ))
      ];
      ...
      // Calculate time thresholds for each rule
      const unitInMinutes = {
        Minutes: 1,
        Hours: 60,
        Days: 1440
      ...
      const orConditions = rules
        .filter(rule => rule.unit in unitInMinutes)
        .map(rule => {
          const minutes = rule.time * unitInMinutes[rule.unit as keyof typeof unitInMinutes];
      ...
      for (const app of activeApps) {
        ...
        for (const rule of rules) {
          if (!(rule.unit in unitInMinutes)) continue;
      ...
      return atRiskResults;
    } catch (error) {
      console.error('Error getting at-risk applications:', error);
      return [];
    }
  }
      ...
      const messages: any[] = [
        {
          ...
          FOR ANY SEARCH QUERY, THE SEARCH NEEDS TO BE CASE INSENSITIVE.
          
          🔍 CASE-INSENSITIVE SEARCH PATTERNS:
          • For name searches: {"name": {"$regex": "admin group", "$options": "i"}}
          • For email searches: {"email": {"$regex": "user@example.com", "$options": "i"}}
          • For hostname searches: {"hostname": {"$regex": "localhost", "$options": "i"}}
          • For environment searches: {"environment": {"$regex": "production", "$options": "i"}}
          • For log message searches: {"message": {"$regex": "error", "$options": "i"}}
          
          🎯 SPECIFIC EXAMPLES:
          • "enlist the members in the admin group" → {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "admin group", "$options": "i"}}}
          • "find the admin group" → {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "admin group", "$options": "i"}}}
          • "show admin group members" → {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "admin group", "$options": "i"}}}
          
          ✅ ALWAYS use $regex with $options: "i" for text field searches to ensure case-insensitive matching
          ❌ NEVER use exact string matching like {"name": "Admin Group"} - this will fail if case doesn't match
...
📊 User Access Summary:
- User Groups: ${userContext.userGroups}
- Assigned Applications: ${userContext.assignedApplications}
- Pinned Applications: ${userContext.pinnedApplications}

Remember to update your own User Acess Summary if you make any changes to user profile, pinned applications, or any other user-related data.
...
Your At-Risk Applications:
${userContext.atRiskApplications.length > 0 ? userContext.atRiskApplications.map(app => `  • ${app.name} - ${app.messages.join(', ')}`).join('\n') : '  • No at-risk applications found'}
` : ''}
...
WHENEVER YOU ADD A NEW APPLICATION, CHECK IF THE APPLICATION IS ALREADY IN THE SYSTEM. IF IT IS, THEN SAY I CANNOT ADD DUPLICATE APPS.
WHENEVER YOU ADD A NEW USER GROUP, CHECK IF THE USER GROUP IS ALREADY IN THE SYSTEM. IF IT IS, THEN SAY I CANNOT ADD DUPLICATE USER GROUPS.
` : `
...
- ALWAYS MAKE TOOL CALLS when necessary!
` : `
🎯 USER-SPECIFIC GUIDANCE: (ONLY FOR NON-ADMIN.)
...
- "My at-risk apps" = use pre-fetched atRiskApplications from context
...
7. "Show my at-risk applications" → 
   Multi-step process:
   Step 1: Get at-risk rules: {"database": "test", "collection": "atriskrules", "filter": {}}
   Step 2: Get user's active applications from user groups
   Step 3: For each rule, calculate time threshold and aggregate logs
   Step 4: Compare log counts against rule thresholds to identify at-risk apps

${userContext.isAdmin ? `
=== 2. NATURAL LANGUAGE APPLICATION & USER GROUP CREATION ===

APPLICATION CREATION PATTERNS:
ANY APPLICATION YOU CREATE MUST ALWAYS BE ADDED TO "admin group" - this is a mandatory requirement
...
5. "Rename 'InventoryService' to 'StockService'" → 
   ...
   {"database": "test", "collection": "applications", "filter": {"name": {"$regex": "InventoryService", "$options": "i"}}, "update": {"$set": {"name": "StockService"}}}

6. "Deactivate 'legacy-app'" → 
   update-many applications:
   {"database": "test", "collection": "applications", "filter": {"name": {"$regex": "legacy-app", "$options": "i"}}, "update": {"$set": {"isActive": false}}}
...
7. "Add 'backend-team@gosaas.io' to 'Ops' group" → 
   ...
   {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "Ops", "$options": "i"}}, "update": {"$addToSet": {"members": {"$oid": "USER_ID"}}}}

8. "Remove 'john.doe@company.com' from 'Development' group" → 
   update-many usergroups:
   {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "Development", "$options": "i"}}, "update": {"$pull": {"members": {"$oid": "USER_ID"}}}}
...
9. "Assign 'StockService' to 'Ops' group" → 
   ...
   update-many usergroups:
   {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "Ops", "$options": "i"}}, "update": {"$addToSet": {"assigned_applications": {"$oid": "APP_ID"}}}}
...
14. "List members in Quality Assurance group" → 
    Step 1: Find group by name (case-insensitive):
    {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "Quality Assurance", "$options": "i"}}}
...
17. "Pin an application" or "Add app to pinned" → 
    Step 1: Find application by name (case-insensitive):
    {"database": "test", "collection": "applications", "filter": {"name": {"$regex": "APP_NAME", "$options": "i"}}}
...
• "Show my at-risk applications" - Multi-step: get rules, get user's active apps, aggregate logs, evaluate against thresholds
...
⚠️ CRITICAL BUSINESS RULES & VALIDATION CONSTRAINTS:

🔒 DUPLICATE PREVENTION RULES:
1. **Application Names**: MUST be unique across the entire system - check for existing names before creating
2. **User Group Names**: MUST be unique across the entire system - check for existing names before creating
3. **User Emails**: MUST be unique across the entire system - check for existing emails before creating

📝 VALIDATION RULES:

**Application Creation/Update:**
- name: 5-20 characters, only letters, numbers, spaces, hyphens (-), underscores (_)
- hostname: max 255 characters, required
- environment: MUST be one of: "Development", "Testing", "Staging", "Production"
- isActive: boolean, required
- description: 10-100 characters, only letters, numbers, spaces, hyphens, underscores, commas, colons, semicolons, parentheses, brackets, quotes

**User Group Creation/Update:**
- name: 5-20 characters, only letters, numbers, spaces, hyphens (-), underscores (_)
- is_admin: boolean, default false
- is_active: boolean, default true
- assigned_applications: array of valid ObjectIds, minimum 1 required for non-admin groups
- members: array of objects with name (2-100 chars) and email (valid email format)

**User Settings:**
- autoRefresh: boolean, required
- logsPerPage: number, required
- autoRefreshTime: number, 15-120 seconds, required

**At Risk Rules:**
- type_of_logs: string, required
- operator: "more" or "less", required
- unit: "Minutes", "Hours", or "Days", required
- time: positive number, required
- number_of_logs: positive number, required

**Data Retention:**
- retentionDays: integer, 1-365 days, required
- updatedBy: valid email, required

🛡️ SECURITY & ACCESS CONTROL RULES:

**User Authentication:**
- Users MUST exist in the database before they can authenticate
- Users MUST be members of at least one active user group to access the system
- Orphaned users (not in any active group) are automatically deleted during authentication
- Only users with valid JWT tokens can access protected endpoints

**Admin Group Rules:**
- Admin groups CANNOT be deleted (only non-admin groups can be deleted)
- Admin groups automatically get access to ALL applications in the system
- Super admin users are automatically added to admin groups and cannot be removed
- Admin groups bypass validation rules for assigned_applications

**Application Assignment Rules:**
- When creating applications, they are automatically assigned to admin groups
- Non-admin groups must have at least one assigned application
- Users can only access applications assigned to their user groups
- When applications are deleted, they are automatically removed from all user groups

**User Group Management:**
- Non-admin groups require at least one assigned application
- Admin groups automatically get all applications
- Users can only be in active groups to access the system
- Group names must be unique across the system

**Pinned Applications:**
- Users can only pin applications they have access to through their user groups
- Invalid pinned applications are automatically cleaned up
- Pinned applications are validated against active applications

**Log Access:**
- Users can only access logs for applications assigned to their user groups
- Log queries are automatically filtered by user's assigned applications
- Case-insensitive search is used for log messages

🔧 OPERATIONAL RULES:

**Data Integrity:**
- When applications are deleted, all associated logs are also deleted
- When applications are deleted, they are removed from all user groups
- User groups maintain referential integrity with applications and users
- Automatic cleanup of invalid pinned applications

**Validation Enforcement:**
- All input data must pass Joi schema validation
...
⚠️ CRITICAL FIELD NAMES (Use exactly as specified):
- Application status: "isActive" (boolean), NOT "status"
- User groups: "members" array (not "userId")
- Applications: "isActive" field (not "status")
- Logs: "log_level" field (not "level")
- User groups: "is_admin" (boolean), "is_active" (boolean)
- Applications: "assigned_applications" array in user groups

✅ AUTOMATIC FIELD HANDLING:
- createdAt, updatedAt, and __v fields are added automatically to all insert operations
- Only include the required business fields in your queries
- The system handles timestamps and versioning automatically
...
- Use case-insensitive searches with $regex and $options: "i"

🔍 SEARCH & FILTERING RULES:
- All text searches must be case-insensitive using $regex with $options: "i"
- Use escapeRegex utility for safe search terms
- Support pagination with page and pageSize parameters
- Support sorting by various fields
- Support filtering by status, environment, and other criteria

🎯 COMMON OPERATION EXAMPLES:

**Creating Applications (Admin Only):**
1. Check for existing name: {"database": "test", "collection": "applications", "filter": {"name": {"$regex": "AppName", "$options": "i"}}}
2. If no existing app, create: {"database": "test", "collection": "applications", "documents": [{"name": "AppName", "hostname": "localhost", "environment": "Development", "isActive": true, "description": "App description"}]}
3. Application is automatically assigned to admin groups

**Creating User Groups (Admin Only):**
1. Check for existing name: {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "GroupName", "$options": "i"}}}
2. If no existing group, create: {"database": "test", "collection": "usergroups", "documents": [{"name": "GroupName", "is_admin": false, "is_active": true, "assigned_applications": [APP_IDS], "members": []}]}

**Adding Users to Groups:**
1. Find user by email: {"database": "test", "collection": "users", "filter": {"email": {"$regex": "user@example.com", "$options": "i"}}}
2. Add to group: {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "GroupName", "$options": "i"}}, "update": {"$addToSet": {"members": {"$oid": "USER_ID"}}}}

**Pinning Applications (User Operations):**
1. Find app by name: {"database": "test", "collection": "applications", "filter": {"name": {"$regex": "AppName", "$options": "i"}}}
2. Add to pinned: {"database": "test", "collection": "users", "filter": {"_id": {"$oid": "USER_ID"}}, "update": {"$addToSet": {"pinned_applications": {"$oid": "APP_ID"}}}}

**Searching with Case-Insensitive Patterns:**
- Find group: {"database": "test", "collection": "usergroups", "filter": {"name": {"$regex": "admin group", "$options": "i"}}}
- Find app: {"database": "test", "collection": "applications", "filter": {"name": {"$regex": "myapp", "$options": "i"}}}
- Find user: {"database": "test", "collection": "users", "filter": {"email": {"$regex": "user@example.com", "$options": "i"}}}

**At-Risk Applications Logic:**
To determine at-risk applications, follow this multi-step process:

1. **Get At-Risk Rules**: {"database": "test", "collection": "atriskrules", "filter": {}}

2. **Get User's Active Applications**:
   - Find user groups: {"database": "test", "collection": "usergroups", "filter": {"members": {"$oid": "USER_ID"}, "is_active": true}}
   - Get assigned apps: Extract assigned_applications from user groups
   - Find active apps: {"database": "test", "collection": "applications", "filter": {"_id": {"$in": [ASSIGNED_APP_IDS]}, "isActive": true}}

3. **Calculate Time Thresholds** for each rule:
   - Minutes: rule.time * 1
   - Hours: rule.time * 60  
   - Days: rule.time * 1440
   - Time threshold = current time - (minutes * 60 * 1000)

4. **Aggregate Logs** for each rule:
   {"database": "test", "collection": "logs", "pipeline": [
     {"$match": {
       "$or": [
         {"log_level": "error", "timestamp": {"$gte": "TIME_THRESHOLD_1"}},
         {"log_level": "warn", "timestamp": {"$gte": "TIME_THRESHOLD_2"}}
       ],
       "application_id": {"$in": [ACTIVE_APP_IDS]}
     }},
     {"$group": {
       "_id": {"application_id": "$application_id", "log_level": "$log_level"},
       "count": {"$sum": 1}
     }}
   ]}

5. **Evaluate Rules** against aggregated results:
   - For each app and log level, compare actual count with rule.number_of_logs
   - Apply rule.operator: "greater", "less", or "equal"
   - If condition is met, app is at-risk

**At-Risk Rule Structure:**
- type_of_logs: string (e.g., "error", "warn", "info")
- operator: "greater", "less", or "equal"
- unit: "Minutes", "Hours", or "Days"
- time: number (how many units back to look)
- number_of_logs: threshold count to trigger the rule

**Example At-Risk Query:**
"Show my at-risk applications" → Multi-step process:
1. Get all at-risk rules
2. Get user's active applications from their user groups
3. For each rule, calculate time threshold and aggregate logs
4. Compare log counts against rule thresholds
5. Return apps that meet any rule conditions

...
class ApplicationViewModel {
    ...
    readonly searchQuery = ko.observable('');
    readonly currentPage = ko.observable(1);
    readonly pageSize = 5;
    readonly totalPages = ko.computed(() => {
        const totalItems = this.applicationDataArray().length;
        return Math.ceil(totalItems / this.pageSize);
    });

    readonly paginatedApplications = ko.computed(() => {
        const query = this.searchQuery().toLowerCase().trim();
        const filtered = this.applicationDataArray().filter(app =>
    ...
    readonly paginatedObservableArray = ko.pureComputed(() =>
        ko.observableArray(this.paginatedApplications())
    );

    readonly paginatedDataProvider = ko.pureComputed(() =>
        new ArrayDataProvider(this.paginatedApplications(), { keyAttributes: "_id" })
    );
...
constructor() {
    ...
    this.currentPage(1); // go back to page 1 on new search
  });
}
